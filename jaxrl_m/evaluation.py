...
def evaluate_with_trajectories(
        policy_fn, high_policy_fn, policy_rep_fn, env: gym.Env, env_name, seed: jax.random.PRNGKey,
        num_episodes: int, base_observation=None, num_video_episodes=0,
        use_waypoints=False, epsilon=0, goal_info=None,
        config=None, single_step_fn=None,
) -> Dict[str, float]:
    rng_key = seed
    ...
    for i in range(num_episodes + num_video_episodes):
        ...
        if 'antmaze' in env_name:
            ...
            from afrl.envs.procgen_viz import get_xy_single
            observation = observation[0]
            obs_goal = goal_img
        elif 'didactic_maze' in env_name:
            obs_goal = env.target_goal.copy()
        else:
        ...
        while not done:
            if not use_waypoints:
                ...
                cur_obs_goal = high_policy_fn(observations=observation, goals=obs_goal, seed=sub_seed)
                if config['use_rep']:
            ...
            if single_step_fn is not None:
                # This is for visualization only
                rng_key, sub_seed = jax.random.split(rng_key)
                cur_one_step = single_step_fn(observations=observation, goals=obs_goal, seed=sub_seed)
                cur_one_step_rep = cur_one_step
                # The following processes should have already been done in the single_step_fn
                # if config['use_rep']:
                #     cur_one_step = cur_one_step / np.linalg.norm(cur_one_step, axis=-1, keepdims=True) * np.sqrt(cur_one_step.shape[-1])
                # else:
                #     cur_one_step = observation + cur_one_step
                # cur_one_step_rep = cur_one_step

            rng_key, sub_seed = jax.random.split(rng_key)
            action = policy_fn(observations=observation, goals=cur_obs_goal_rep, low_dim_goals=True, seed=sub_seed)
            if 'antmaze' in env_name:
                ...
                # next_observation, r, done, info = env.step({'ac': np.array(action)})  # We handle this in CalvinWrapper
                next_observation, r, done, info = env.step(action)
                ...
                if i >= num_episodes and step % 3 == 0:
                    if 'antmaze' in env_name:
                        ...
                        if use_waypoints and not config['use_rep'] and ('large' in env_name or 'ultra' in env_name):
                            ...
                            # Plot one-step subgoal
                            # if single_step_fn is not None:
                            #     one_step_x, one_step_y = cur_one_step_rep[:2]
                            #     pixx, pixy = xy_to_pixxy(one_step_x, one_step_y)
                            #     cur_frame[0, int((pixy - 0.02) * size):int((pixy + 0.02) * size), int((pixx - 0.02) * size):int((pixx + 0.02) * size)] = 0
                            #     cur_frame[1, int((pixy - 0.02) * size):int((pixy + 0.02) * size), int((pixx - 0.02) * size):int((pixx + 0.02) * size)] = 255
                            #     cur_frame[2, int((pixy - 0.02) * size):int((pixy + 0.02) * size), int((pixx - 0.02) * size):int((pixx + 0.02) * size)] = 0
                        render.append(cur_frame)
                    elif 'kitchen' in env_name:
