...
def get_xy(obs, obj: str = 'mouse'):
    if obj == 'mouse':
        grey = np.array([187, 203, 204])    # The pixel of mouse
        dists = np.abs(obs - grey).sum(axis=-1)
        return np.stack([
            dists.min(-2).argmin(-1), dists.min(-1).argmin(-1)
        ], axis=1)
    elif obj == 'cheese':
        yellow = np.array([250, 231, 119])  # The pixel of cheese
        dists = np.abs(obs - yellow).sum(axis=-1)
        return np.stack([
            dists.min(-2).argmin(-1), dists.min(-1).argmin(-1)
        ], axis=1)
    else:
        raise NotImplementedError(f"Unknown object {obj}")
...
def get_all_states(level):
    ...
    done_states = set()     # The position at the step before the mouse reaches the cheese
    ...
    env.close()
    return seen_states, edges, done_states
...
class ProcgenLevel:
    ...
    def create(cls, level_id):
        all_states, all_edges, done_states = get_all_states(level_id)
        ...
        return cls(all_states, all_locs, all_imgs, done_states, tuple_locs, all_edges, distances, optimal_actions)
